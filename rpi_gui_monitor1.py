...
class AlgaeMonitorApp:
    def __init__(self, root):
        ...
        if not os.path.exists(CSV_FILE):
            with open(CSV_FILE, mode='w', newline='') as f:
                csv.writer(f).writerow(["Timestamp", "Device", "Sensor", "Value"])
    ...
    def save_to_buffer(self, timestamp, device_id, label, value):
        """將數據存入緩衝區，並在必要時寫入檔案"""
        with self.buffer_lock:
            self.data_buffer.append([timestamp, device_id, label, value])
    ...
    def sync_to_cloud(self, device_id, label, value):
        """背景傳送數據到 Google Sheets"""
                            ...
                            for key in self.sensor_keys:
                                if key in v:
                                    ...
                                    if self.status[key].get():
                                        ...
                                        self.save_to_buffer(ts, device_id, self.labels[key], val)
                                        # 同步至雲端
                                        self.sync_to_cloud(device_id, self.labels[key], val)
                                    else:
