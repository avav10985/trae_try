...
void loop() {
  ...
  // 溫度補償：EC 隨溫度增加而增加，補償係數約 2%/°C
  float ecCompensationCoeff = 1.0 + 0.02 * (currentTemp - 25.0);
  float ecCompensated = ecRaw / ecCompensationCoeff; // 補償後的值
  v["ec"] = (int)ecCompensated; // 建議校準後換算成 ms/cm

  // 5. 濁度 (Turbidity) - 通常不需要溫度補償，但若需要可添加
  ...
  // 7. CO2 (交替讀取 MH-Z19B & C) - MH-Z19 內建補償，通常不需要額外補償
