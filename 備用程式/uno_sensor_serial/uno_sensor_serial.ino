...
void loop() {
  ...
  float ecValue = -1.0f;  // -1 表示斷線
  if (ecAvgADC >= 5) {
    float ecVoltage_mV = ecAvgADC * (5000.0 / 1023.0);
    float ecRaw = ecVoltage_mV / 164.0;
    float ecTempCoeff = 1.0 + 0.0185 * (currentTemp - 25.0);
    ecValue = ecRaw * EC_K / ecTempCoeff;
  }
  v["ec"] = ecValue;

  // 4b. TDS(EC):由 EC 推算的 TDS,適用於海水(SEN0244 量程不夠用)
  // 公式:TDS (ppm) ≈ EC (mS/cm) × 1000 × 0.65 (海水 NaCl 換算因子)
  // 海水 EC ~50 mS/cm 對應 TDS ~32500 ppm
  v["tdse"] = (ecValue < 0) ? -1 : (int)(ecValue * 1000.0 * 0.65);
  ...
  delay(10000); // 等 10 秒,加上感測器讀取時間(~4 秒)實際週期約 14 秒
}
